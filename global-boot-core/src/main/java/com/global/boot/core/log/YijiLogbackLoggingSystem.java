...
public class YijiLogbackLoggingSystem extends LogbackLoggingSystem {
	private ClassLoader classLoader;
	
	public YijiLogbackLoggingSystem(ClassLoader classLoader) {
		super(classLoader);
	...
	public void initialize(LoggingInitializationContext initializationContext, String configLocation, LogFile logFile) {
		//提前parse配置格式
		new MessageReplaceConverter();
		defaultConverterMap.put("m", MessageReplaceConverter.class.getName());
		defaultConverterMap.put("msg", MessageReplaceConverter.class.getName());
		defaultConverterMap.put("message", MessageReplaceConverter.class.getName());
	...
	protected void loadDefaults(LoggingInitializationContext initializationContext, LogFile logFile) {
		...
		context.stop();
		context.reset();
		LogbackConfigurator configurator = new LogbackConfigurator(context, this.classLoader,
	...
	private void customLog(LogbackConfigurator configurator) {
		...
		List<LogInitializer> logInitializers = SpringFactoriesLoader.loadFactories(LogInitializer.class,
			getClassLoader());
		for (LogInitializer logInitializer : logInitializers) {
			logInitializer.init(configurator);
		}
		YijiApplicationRunListener.AppBanner.getInfos().addAll(configurator.getLogs());
	}
	...
	private void assertEnv(Environment environment) {
		if (environment instanceof ConfigurableEnvironment) {
			...
			PropertySource ps = mutablePropertySources
	...
	protected void loadConfiguration(LoggingInitializationContext initializationContext, String location,
		...
		//渲染异常日志时，忽略的栈
		System.setProperty("yiji.log.ignoredStackFrames", "java.lang.reflect.Method," + "org.apache.catalina,"
		...
		LogbackConfigurator configurator = new LogbackConfigurator(loggerContext, this.classLoader,
			initializationContext.getEnvironment());
	...
	private LoggerContext getLoggerContext() {
		...
		Assert.isInstanceOf(LoggerContext.class, factory, String.format(
			"LoggerFactory is not a Logback LoggerContext but Logback is on "
					...
					+ "prefer-application-packages in WEB-INF/weblogic.xml", factory.getClass(), getLocation(factory)));
